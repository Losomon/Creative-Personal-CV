export const svg = (paths, options = {}) => {
  const {
    width = 24,
    height = 24,
    viewBox = '0 0 24 24',
  ...
  return `<svg viewBox="${viewBox}" width="${width}" height="${height}" ${strokeAttr} ${fillAttr} ${strokeWidthAttr} class="${className}"><g>${paths}</g></svg>`
}
...
export const ICONS = {
  ...
  clockAlt: `<circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>`,
...
export const renderIcon = (name, options = {}) => {
  const iconSvg = ICONS[name]
  if (!iconSvg) return ''
  return svg(iconSvg, options)
}
